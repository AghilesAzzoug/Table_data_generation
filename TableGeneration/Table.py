...
class Table:
    ...
    def __init__(self,no_of_rows,no_of_cols,images_path,ocr_path,gt_table_path,assigned_category,distributionfile):
        ...
        self.cells_idcounter = 0
        # self.rows_idcounter = 0
        # self.cols_idcounter = 0
    ...
    def generate_random_text(self, type):
        ...
        for e in out:
            html+='<span id='+str(self.cells_idcounter)+'>'+str(e)+' </span>'
            ids.append(self.cells_idcounter)
            self.cells_idcounter+=1
    ...
    def create_same_col_matrix(self):
        ...
        for col in range(self.no_of_cols):
            ...
            self.all_cols.append(single_col)
        return self.create_same_matrix(self.all_cols, self.cells_idcounter)
    ...
    def create_same_row_matrix(self):
        ...
        for row in range(self.no_of_rows):
            ...
            for subarr in self.data_matrix[row,:]:
                if(subarr is not None):
                    single_row+=subarr
            self.all_rows.append(single_row)
        return self.create_same_matrix(self.all_rows, self.cells_idcounter)
        ...
        for row in range(self.no_of_rows):
            for col in range(self.no_of_cols):
                if(self.data_matrix[row,col] is not None):
                    self.all_cells.append(self.data_matrix[row,col])
        return self.create_same_matrix(self.all_cells, self.cells_idcounter)
    ...
    def create(self):
        ...
        return cells_matrix, cols_matrix, rows_matrix, self.cells_idcounter, html, tablecategory



if __name__ == "__main__":
    table = Table(no_of_rows=5, no_of_cols=6,images_path='../images',ocr_path='../unlv_xml_ocr',
          gt_table_path='../unlv_xml_gt',
          assigned_category=0,distributionfile='../unlv_distribution')

    cells_matrix,cols_matrix,rows_matrix,cells_idcounter,html,tablecategory = table.create()

    print(cells_matrix.shape)
    print(cols_matrix.shape)
    print(rows_matrix.shape)
