...
class GenerateTFRecord:
    def __init__(self, outpath,filesize,unlvimagespath,unlvocrpath,unlvtablepath,visualizeimgs,visualizebboxes,distributionfilepath):
        ...
        self.visualizeimgs=True#visualizeimgs                #wheter to store images separately or not
        ...
        self.tables_cat_dist = [2,2,2,2]#self.get_category_distribution(self.filesize)
        print("self.tables_cat_dist : ", self.tables_cat_dist)
        self.visualizebboxes=True #visualizebboxes
    ...
    def generate_tables(self,driver,N_imgs,output_file_name):
        ...
        for assigned_category,cat_count in enumerate(self.tables_cat_dist):
            print("assigned_category,cat_count : ", assigned_category, cat_count)
...
if __name__ == '__main__':
    g = GenerateTFRecord(outpath='../tfrecords',filesize=1,
                         unlvimagespath='../images',
                         unlvocrpath='../unlv_xml_ocr',
                         unlvtablepath='../unlv_xml_gt',
                         visualizeimgs=True,visualizebboxes=True,
                         distributionfilepath='../unlv_distribution')
    opts = Options()
    opts.set_headless()
    driver = Firefox(options=opts)

    data_arr, all_table_categories = g.generate_tables(driver,N_imgs=10,output_file_name="aghiles")

    print(data_arr)

    print(all_table_categories)